...
def _check_password(user_id, user_email, password, password_new, db_config):
    salt = bcrypt.gensalt()
    password_encode = password.encode('utf-8')
    password_hashed = bcrypt.checkpw(password_encode, salt)
    check_password = database_access_functions._login(user_email, password_hashed, db_config)
